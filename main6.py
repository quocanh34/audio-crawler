...
def main():
    ...
    youtube_df = pd.read_csv(current_dir + config_env["CSV_LINK6"])
    ...
    for index, row in youtube_df.iterrows():
        try:
            ...
            if (index+1) % 10 == 0:
                final_dataset.push_to_hub(config_env["HUGGINGFACE_HUB"] + f"split6_vid_{index+1}", token=config_env["TOKEN"])
                print("-"*10)
    ...
    final_dataset.push_to_hub(config_env["HUGGINGFACE_HUB"] +"split6_final", token=config_env["TOKEN"])
    print(final_dataset)
main()
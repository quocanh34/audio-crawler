...
def main():
    ...
    for index, row in youtube_df.iterrows():
        try:
            ...
            if (index+1) % 10 == 0:
                final_dataset.push_to_hub(config_env["HUGGINGFACE_HUB"] + f"split7_vid_{index+1}", token=config_env["TOKEN"])
                print("-"*10)
    ...
    final_dataset.push_to_hub(config_env["HUGGINGFACE_HUB"] +"split7_final", token=config_env["TOKEN"])
    print(final_dataset)
main()